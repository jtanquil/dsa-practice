...
#define POLYNOMIAL_CONSTANT 256
...
int poly_hash(char *key, int mod) {
  int i = 0;

  for (; *key != '\0'; key++) {
    i = (*key + POLYNOMIAL_CONSTANT * i) % mod;
  }
...
int hash_1(char *key) {
  return poly_hash(key, TABLE_SIZE);
}

int hash_2(char *key) {
  return 1 + poly_hash(key, (TABLE_SIZE - 1) / 2);
}

int hash(char *key, int iteration) {
  return (hash_1(key) + iteration * hash_2(key)) % TABLE_SIZE;
}
